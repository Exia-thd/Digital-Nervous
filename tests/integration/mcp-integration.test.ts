...
describe('MCP Server Integration', () => {
  ...
  describe('Error Handling', () => {
    it('Digital-NervousError serializes correctly', async () => {
      const { Digital-NervousError, ErrorCode } = await import('../../mcp/build/errors.js');
      const err = new Digital-NervousError(ErrorCode.TOOL_NOT_FOUND, 'Tool missing', { tool: 'test' });
      const json = err.toJSON();
      expect(json.code).toBe('FW301');
      expect(json.context).toBeDefined();
    });

    it('isDigital-NervousError detects error types', async () => {
      const { isDigital-NervousError, Digital-NervousError, ErrorCode } = await import('../../mcp/build/errors.js');
