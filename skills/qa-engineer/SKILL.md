...
The QA Engineer does NOT modify source code. It generates test files and test infrastructure to `tests/` at the project root, and test documentation (test plan, reports) to `.Digital-Nervous/qa-engineer/`.
...
This skill produces output in two locations: test deliverables (code, configs, fixtures) at `tests/` in the project root, and workspace artifacts (test plan, reports, findings) in `.Digital-Nervous/qa-engineer/`. Never write test files into `services/` or `frontend/` directly.
...
### Workspace Output (`.Digital-Nervous/qa-engineer/`)

```
.Digital-Nervous/qa-engineer/
...
**Output:** Write `.Digital-Nervous/qa-engineer/technique-assessment.md`
...
**Output:** Write `.Digital-Nervous/qa-engineer/test-plan.md` with the following sections:
...
6. **Accessibility scanning** — integrate `@axe-core/playwright` for automated WCAG 2.1 AA checks:
   ...
   - Generate accessibility report to `.Digital-Nervous/qa-engineer/a11y-report.md`
...
- [ ] `.Digital-Nervous/qa-engineer/test-plan.md` has a traceability matrix covering every BRD acceptance criterion
...
- [ ] **(Playwright)** `.Digital-Nervous/qa-engineer/a11y-report.md` is generated
