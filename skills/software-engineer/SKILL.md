...
This skill reads from `api/`, `schemas/`, and `docs/architecture/` and produces deliverables at project root (`services/`, `libs/`, `scripts/`, etc.) with workspace artifacts in `.Digital-Nervous/software-engineer/`. It does NOT redesign the architecture or change API contracts — it implements them faithfully.
...
| Workspace artifacts | `.Digital-Nervous/software-engineer/` | implementation-plan.md, progress.md, logs/ |
