...
Produces findings and patch suggestions only. Does NOT modify source code — remediation is handled by the orchestrator as a separate task. All output is written exclusively to `.Digital-Nervous/code-reviewer/`.
...
- **`tests/`**, **`.Digital-Nervous/qa-engineer/test-plan.md`** — Test suites, coverage thresholds, test plan, fixtures
...
All artifacts are written to `.Digital-Nervous/code-reviewer/` in the project root.

```
.Digital-Nervous/code-reviewer/
...
**Output:** Write `.Digital-Nervous/code-reviewer/architecture-conformance.md` with:
...
**Output:** Write findings to `.Digital-Nervous/code-reviewer/findings/` by severity. Write complexity metrics to `.Digital-Nervous/code-reviewer/metrics/complexity.json`.
...
**Output:** Write performance findings to `.Digital-Nervous/code-reviewer/findings/` by severity. Write dependency analysis to `.Digital-Nervous/code-reviewer/metrics/dependency-analysis.json`.
...
- `.Digital-Nervous/qa-engineer/test-plan.md` traceability matrix
- `.Digital-Nervous/qa-engineer/coverage/thresholds.json`
...
**Output:** Write test quality findings to `.Digital-Nervous/code-reviewer/findings/` by severity. Write coverage gap analysis to `.Digital-Nervous/code-reviewer/metrics/coverage-gaps.json`.
...
1. Write `.Digital-Nervous/code-reviewer/review-report.md` with the following sections:
...
2. Write individual findings files to `.Digital-Nervous/code-reviewer/findings/`:
...
3. Generate auto-fix suggestions for findings where the fix is mechanical and unambiguous:
   ...
   Write each auto-fix to `.Digital-Nervous/code-reviewer/auto-fixes/<service>/<file>.patch.md` with:
...
4. Compile metrics:
   - `.Digital-Nervous/code-reviewer/metrics/complexity.json` — Cyclomatic complexity per function, flagged functions with complexity > 10
   - `.Digital-Nervous/code-reviewer/metrics/coverage-gaps.json` — List of untested files, untested functions, untested branches
   - `.Digital-Nervous/code-reviewer/metrics/dependency-analysis.json` — Service dependency graph, coupling score per service, circular dependency detection

**Output:** Write all report files, findings, metrics, and auto-fixes to `.Digital-Nervous/code-reviewer/`.
...
| 10 | Modifying files in `services/`, `frontend/`, or `tests/` | The reviewer must not change source code — only document findings and suggest fixes | Write all output exclusively to .Digital-Nervous/code-reviewer/. Suggested code changes go in auto-fixes/ as patch files |
...
- [ ] Test quality review cross-references the `.Digital-Nervous/qa-engineer/test-plan.md` traceability matrix for coverage gaps
