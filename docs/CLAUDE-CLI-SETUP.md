# Digital-Nervous — Hướng Dẫn Cài Đặt cho Claude CLI

> **Mục tiêu:** Biến Digital-Nervous thành một bộ công cụ sản xuất phần mềm chuyên nghiệp tích hợp vào Claude CLI, với 56 kỹ năng AI, pipeline 6 pha, và 4 cấp độ power tùy nhu cầu.
...
**Digital-Nervous là gì?**

Digital-Nervous là một orchestrator (người điều phối) với 56 kỹ năng AI chuyên biệt, bao phủ toàn bộ vòng đời phát triển phần mềm:
...
**Kết quả:** File `.Digital-Nervous/memory.jsonl` được tạo.
...
2. Tạo `.Digital-Nervous/mcp-server/` với TypeScript server
...
```json
{
  "mcpServers": {
    "Digital-Nervous": {
      "command": "npx",
      "args": ["tsx", "/path/to/Digital-Nervous/.Digital-Nervous/mcp-server/server.ts"]
    }
  }
}
```
...
```json
{
  "mcpServers": {
    "my-project": {
      "command": "npx",
      "args": ["tsx", ".Digital-Nervous/mcp-server/server.ts"]
    }
  }
}
```
...
```bash
echo "=== Digital-Nervous Verification ==="
...
echo "ForgeNexus:      $([ -d .Digital-Nervous/mcp-server ] && echo 'OK' || echo 'MISSING')"
...
# Commit thay đổi
git add .
git commit -m "chore: update Digital-Nervous"
```
...
```yaml
# .github/workflows/Digital-Nervous.yml
- name: ForgeNexus Analyze
  uses: buiphucminhtam/Digital-Nervous-actions/analyze@main

- name: PR Blast Radius
  uses: buiphucminhtam/Digital-Nervous-actions/pr-review@main
  with:
    base: main
