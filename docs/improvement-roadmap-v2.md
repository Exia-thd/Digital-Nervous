# Digital-Nervous — Improvement Roadmap v2: ≥ 9/10 Scored Items

> Scored against 8 criteria with weighted average.
> **Target threshold: ≥ 9.0/10**
> All improvements slot into existing Digital-Nervous flows without architectural rewrites.
...
| 4 | **Architecture Fit** | 10% | 10 | Compatibility with existing Digital-Nervous flows |
...
- RTK: `Digital-Nervous-mcp-setup.sh` detection
...
```bash
# In Digital-Nervous-mcp-setup.sh — add RTK detection
detect_shell_compressor() {
  ...
  if command -v rtk &> /dev/null; then
    ...
    SHELL_COMPRESSOR="Digital-Nervous-shell-filter"
  ...
  echo "SHELL_COMPRESSOR=${SHELL_COMPRESSOR}" >> "${PROJECT_ROOT}/.Digital-Nervous/settings.env"
}

# Middleware delegation:
run_shell_filter() {
  source "${PROJECT_ROOT}/.Digital-Nervous/settings.env" 2>/dev/null
...
**What:** Detect RTK at setup time. If present, delegate all shell filtering to RTK (60-90% reduction). If not present, use native `Digital-Nervous-shell-filter.sh` (60-80% reduction). Zero user-facing change. Automatic upgrade path.
...
```bash
# Digital-Nervous-mcp-setup.sh — RTK detection block
if command -v rtk &> /dev/null; then
  ...
  "servers": [
    ...
    },
    {
      ...
      "env": {
        ...
        "TOKEN_SAVIOR_CLIENT": "Digital-Nervous"
      },
      "auto_start": true
    }
  ]
...
```bash
# In Digital-Nervous-mcp-setup.sh — Token-Savior detection
