# Digital-Nervous — Scored Improvement Roadmap

> Scored against 8 criteria. Target threshold: **≥ 7.0/10** per improvement.
> All improvements must slot into existing Digital-Nervous flows without architectural rewrites.
...
| 4 | **Architecture Fit** | Compatibility with existing Digital-Nervous flows |
| 5 | **Reliability** | Production maturity, error handling, edge cases |
| 6 | **Maintainability** | Dependencies, complexity, future-proofing |
| 7 | **Scope Impact** | How many Digital-Nervous components are affected |
...
### I1: Shell Output Filter (Digital-Nervous-native)
...
```bash
# scripts/Digital-Nervous-shell-filter.sh
...
if command -v rtk &> /dev/null; then
    ...
    Digital-Nervous-shell-filter.sh "$@"
fi
```
...
// Stored in .Digital-Nervous/tool-audit.jsonl (not in context)
```
...
**DCP comparison:** DCP does this at the OpenCode hook level. Digital-Nervous implements it at the middleware level, making it platform-agnostic.
...
**What:** Detect RTK at install time; if present, delegate shell filtering to RTK. If not present, use native `Digital-Nervous-shell-filter.sh`. Zero user-facing change.

**Existing flow it hooks into:** `scripts/Digital-Nervous-mcp-setup.sh` — RTK detection added to setup.

**Implementation:**

```bash
# In Digital-Nervous-mcp-setup.sh — add RTK check
check_rtk() {
  if command -v rtk &> /dev/null; then
    log_ok "RTK detected: shell output compression enabled (60-90% reduction)"
    echo "rtk_enabled=true" >> "${PROJECT_ROOT}/.Digital-Nervous/settings.env"
  else
    log_info "RTK not found. Install for 60-90% shell output compression:"
    log_info "  brew install rtk  # or: cargo install rtk"
    echo "rtk_enabled=false" >> "${PROJECT_ROOT}/.Digital-Nervous/settings.env"
  fi
}

# In middleware — delegate to RTK if available
run_shell_filter() {
  if [[ -f "${PROJECT_ROOT}/.Digital-Nervous/settings.env" ]] && \
     grep -q "rtk_enabled=true" "${PROJECT_ROOT}/.Digital-Nervous/settings.env"; then
    rtk "$@"
  else
    Digital-Nervous-shell-filter.sh "$@"
  fi
...
```
.Digital-Nervous/mcp-manifest.json
...
**What:** Use a small, cheap LLM to compress conversation spans into summaries. Inspired by DCP's message compression but implemented as a Digital-Nervous middleware.
