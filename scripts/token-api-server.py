    ...
    def get_summary(self) -> dict:
        ...
        summary = {
            "platforms": {
                ...
                "Digital-Nervous": {"available": False, "calls": 0, "tokens": 0, "cost": 0},
            },
            "by_model": {},
...
class TokenAPI:
    def __init__(self):
        ...
        self.base_path = Path(self.home) / '.Digital-Nervous' / 'usage'
...
def create_app():
    ...
    if HAS_FLASK:
        ...
        def unified_usage():
            ...
            cursor_reader = CursorDBReader()
            project = request.args.get('project', 'Digital-Nervous')
            period = int(request.args.get('period', 7))
            Digital-Nervous_data = api.get_usage(project, period)

            cursor_models_data = cursor_reader.get_model_stats()

            return jsonify({
                'Digital-Nervous': Digital-Nervous_data,
                'cursor': {
    ...
    class Handler(http.server.SimpleHTTPRequestHandler):
        def do_GET(self):
            if self.path == '/api/projects':
                ...
                import urllib.parse
                qs = urllib.parse.parse_qs(urllib.parse.urlparse(self.path).query)
                project = qs.get('project', ['Digital-Nervous'])[0]
                Digital-Nervous_data = api.get_usage(project, 7)
                self.send_response(200)
                self.send_header('Content-type', 'application/json')
                self.end_headers()
                self.wfile.write(json.dumps({
