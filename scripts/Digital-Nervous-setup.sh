...
#
# USAGE:
#   bash Digital-Nervous-setup.sh              # Full setup
#   bash Digital-Nervous-setup.sh --check      # Check status
#   bash Digital-Nervous-setup.sh --diagnose   # Diagnostics
#   bash Digital-Nervous-setup.sh --force       # Reinstall everything
# ═══════════════════════════════════════════════════════════════════════════════

set -euo pipefail

VERSION="1.0.0"
GITHUB_REPO="https://github.com/buiphucminhtam/Digital-Nervous.git"
FORGEWRIGHT_DEFAULT="${HOME}/Documents/GitHub/Digital-Nervous"
MIN_NODE_VERSION=18
...
detect_fw_dir() {
    ...
    local dirs=(
        "${FORGEWRIGHT_DEFAULT}"
        "${HOME}/Projects/Digital-Nervous"
        "${HOME}/code/Digital-Nervous"
        "$(dirname "$resolved")/Digital-Nervous"
        "$(dirname "$resolved")"
    )
...
{
  "Digital-Nervous_version": "${VERSION}",
  "generated_at": "$(date -u +%Y-%m-%dT%H:%M:%SZ)",
  "workspace": "${project_root}",
  "Digital-Nervous_path": "${fw_dir}"
}
EOF
...
USAGE:
  Digital-Nervous-setup.sh [OPTIONS] [PROJECT_PATH]
...
EXAMPLES:
  ...
  bash Digital-Nervous-setup.sh --check         # Check status
  bash Digital-Nervous-setup.sh --diagnose     # Diagnostics
  bash Digital-Nervous-setup.sh --install-fw   # Install FW then setup
EOF
}
...
main() {
    ...
    if [[ "$install_fw" == "true" ]] || [[ -z "$fw_dir" ]]; then
        fw_dir=$(ensure_Digital-Nervous "$fw_dir")
    fi
