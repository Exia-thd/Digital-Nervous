#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────
# Digital-Nervous-mcp-setup — One-Command MCP Setup (v2)
#
...
#   bash scripts/Digital-Nervous-mcp-setup.sh --force
#   bash scripts/Digital-Nervous-mcp-setup.sh --diagnose
# ─────────────────────────────────────────────────────────
...
detect_paths() {
    ...
    # Find Digital-Nervous directory
    if [[ "$resolved" == */scripts ]]; then
        ...
        if [[ -d "$plugin_root/Digital-Nervous" ]]; then
            FORGEWRIGHT_DIR="$plugin_root/Digital-Nervous"
        else
    ...
    # If Digital-Nervous is a subdirectory of PWD, PWD is the host project
    if [[ "$FORGEWRIGHT_DIR" == "$PROJECT_ROOT"/* ]]; then
        # Digital-Nervous is a submodule
...
check_prerequisites() {
    ...
    # Check Digital-Nervous structure
    if [[ ! -f "${FORGEWRIGHT_DIR}/AGENTS.md" ]] && [[ ! -f "${FORGEWRIGHT_DIR}/CLAUDE.md" ]]; then
        log_warn "Digital-Nervous structure not recognized at: $FORGEWRIGHT_DIR"
    fi
...
setup_mcp_server() {
    ...
    local server_dir="${PROJECT_ROOT}/.Digital-Nervous/mcp-server"
    local manifest="${PROJECT_ROOT}/.antigravity/mcp-manifest.json"

    # Create directories
    mkdir -p "${PROJECT_ROOT}/.Digital-Nervous"
    mkdir -p "${PROJECT_ROOT}/.antigravity"
    mkdir -p "$server_dir"

    # Copy Digital-Nervous MCP server
    ...
    cat > "$manifest" <<EOF
{
  ...
  "Digital-Nervous_path": "${FORGEWRIGHT_DIR}",
  "servers": [
    {
      "name": "Digital-Nervous",
      "type": "forgenexus",
      "path": "${server_dir}"
    }
  ]
...
create_launcher() {
    local launcher="${PROJECT_ROOT}/.Digital-Nervous/mcp-launcher.sh"

    cat > "$launcher" <<'LAUNCHER'
#!/usr/bin/env bash
# MCP Launcher for ForgeNexus
# Auto-generated by Digital-Nervous-mcp-setup.sh
...
# Find Digital-Nervous
if [[ -f "$WORKSPACE/Digital-Nervous/.cursor/forgenexus-mcp.js" ]]; then
    FORGEWRIGHT="$WORKSPACE/Digital-Nervous"
elif [[ -f "$WORKSPACE/.cursor/forgenexus-mcp.js" ]]; then
    FORGEWRIGHT="$WORKSPACE"
elif [[ -f "${HOME}/Documents/GitHub/Digital-Nervous/.cursor/forgenexus-mcp.js" ]]; then
    FORGEWRIGHT="${HOME}/Documents/GitHub/Digital-Nervous"
else
    echo "ERROR: Digital-Nervous not found" >&2
    exit 1
fi
...
update_global_config() {
    ...
    local server_name="Digital-Nervous-$(basename "$PROJECT_ROOT" | tr '[:upper:]' '[:lower:]' | tr -cd '[:alnum:]-_' || echo 'workspace')"
    local launcher="${PROJECT_ROOT}/.Digital-Nervous/mcp-launcher.sh"
...
verify_setup() {
    ...
    # Check launcher
    ((total++))
    if [[ -f "${PROJECT_ROOT}/.Digital-Nervous/mcp-launcher.sh" ]]; then
        ((passed++))
    ...
    if [[ -f "$config_path" ]] && grep -q "Digital-Nervous" "$config_path" 2>/dev/null; then
        ((passed++))
        log_ok "Global config updated"
    else
...
console.log('  Version: ' + m.Digital-Nervous_version);
...
" 2>/dev/null
    ...
    if [[ -f "$config_path" ]]; then
        if grep -q "Digital-Nervous" "$config_path" 2>/dev/null; then
            log_ok "Global config: ✓"
        else
            log_warn "Global config: (no Digital-Nervous entry)"
        fi
    else
...
cmd_diagnose() {
    ...
    for f in \
        ...
        "${PROJECT_ROOT}/.Digital-Nervous/mcp-launcher.sh" \
...
show_help() {
    cat << 'EOF'
Digital-Nervous-mcp-setup — One-command MCP setup for any project

USAGE:
  Digital-Nervous-mcp-setup.sh [OPTIONS]
...
EXAMPLES:
  ...
  Digital-Nervous-mcp-setup.sh

  # Check status
  Digital-Nervous-mcp-setup.sh --check

  # Force re-generate
  Digital-Nervous-mcp-setup.sh --force

  # Diagnose issues
  Digital-Nervous-mcp-setup.sh --diagnose
EOF
}
...
main() {
    ...
    case "$mode" in
        ...
        uninstall)
            ...
            rm -rf "${PROJECT_ROOT}/.Digital-Nervous/mcp-server"
