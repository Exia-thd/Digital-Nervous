...
# Usage: Digital-Nervous-mcp [start|stop|status|config|manifest|launcher]
#
# Supports two modes:
...
# ─── Detect Digital-Nervous Submodule Path ────────────────────

detect_Digital-Nervous_path() {
    # If Digital-Nervous IS this repo, return its path
    if [ -f "${PROJECT_ROOT}/.Digital-Nervous/version" ] || [ -f "${PROJECT_ROOT}/VERSION" ]; then
        echo "$PROJECT_ROOT"
        return
    fi
    ...
    if [ -f "${PROJECT_ROOT}/.git" ]; then
        local fw_path
        fw_path=$(git -C "$PROJECT_ROOT" config --get submodule.Digital-Nervous.path 2>/dev/null || true)
        if [ -n "$fw_path" ]; then
    ...
    # Fallback: look for Digital-Nervous directory
    if [ -d "${PROJECT_ROOT}/Digital-Nervous" ]; then
        echo "${PROJECT_ROOT}/Digital-Nervous"
        return
    fi
...
cmd_status() {
    echo ""
    echo "━━━ Digital-Nervous MCP Status ━━━"
    echo ""
...
console.log('  Digital-Nervous:', m.Digital-Nervous_version);
console.log('');
console.log('  Servers:');
...
case "${1:-help}" in
    ...
    *)
        echo "Digital-Nervous-mcp — MCP Server & Workspace Manager"
        echo ""
        echo "Usage: Digital-Nervous-mcp <command>"
        echo ""
        echo "Commands:"
