...
(
    ...
    elif [ -f "$PROJECT_ROOT/.Digital-Nervous/project-profile.json" ]; then
        PROJECT_CONTEXT=$(head -n 50 "$PROJECT_ROOT/.Digital-Nervous/project-profile.json")
    ...
    python3 /root/scripts/Digital-Nervous-orchestrator.py "$PROJECT" "$TASK" "$CODE_DIR" 2>&1 | tee "$EXEC_LOG"
