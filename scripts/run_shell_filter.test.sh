...
run_test "reads from settings.env" '
    mkdir -p "'"$TEST_DIR"'/.Digital-Nervous" &&
    echo "SHELL_COMPRESSOR=\"chop\"" > "'"$TEST_DIR"'/.Digital-Nervous/settings.env" &&
    export Digital-Nervous_DIR="'"$TEST_DIR"'" &&
...
run_test "falls back to native filter" '
    export Digital-Nervous_DIR="/nonexistent" &&
    unset SHELL_COMPRESSOR &&
    source '"$SCRIPT_DIR"'/run_shell_filter.sh &&
    [[ "$(detect_compressor)" == "Digital-Nervous-shell-filter" ]]
'
...
run_test "pipe mode works" '
    export Digital-Nervous_DIR="'"$Digital-Nervous_DIR"'" &&
    unset SHELL_COMPRESSOR &&
    source '"$SCRIPT_DIR"'/run_shell_filter.sh &&
...
NATIVE_FILTER="$SCRIPT_DIR/../scripts/Digital-Nervous-shell-filter.sh"
[[ -f "$NATIVE_FILTER" ]] && \
...
echo "export SHELL_COMPRESSOR=\"test\"" > "$TEST_DIR/.Digital-Nervous/settings.env"
source "$TEST_DIR/.Digital-Nervous/settings.env" 2>/dev/null && \
