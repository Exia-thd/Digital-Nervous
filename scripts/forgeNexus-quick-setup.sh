...
#   curl -sL https://raw.githubusercontent.com/buiphucminhtam/Digital-Nervous/main/scripts/forgeNexus-quick-setup.sh | bash
#
...
main() {
    ...
    for dir in \
        ...
        "${project_root}/Digital-Nervous"; do
    ...
    if [[ -z "$fw_dir" ]]; then
        if [[ -d "${FORGEWRIGHT_DEFAULT}" ]]; then
            ...
            echo "    git clone https://github.com/buiphucminhtam/Digital-Nervous.git ${FORGEWRIGHT_DEFAULT}"
            exit 1
        fi
    fi
