...
#       "Digital-Nervous": {
#         "command": "bash",
#         "args": ["/path/to/scripts/Digital-Nervous-mcp-launcher.sh"]
#       }
#     }
#   }
# ─────────────────────────────────────────────────────────────────
...
find_manifest() {
    ...
    elif [[ -f "$workspace/.Digital-Nervous/mcp-manifest.json" ]]; then
        echo "$workspace/.Digital-Nervous/mcp-manifest.json"
    else
        echo ""
    fi
}
...
resolve_server_cmd() {
    ...
    if [[ -z "$manifest" ]] || ! validate_manifest_path "$manifest" "$workspace"; then
        local local_server="$workspace/.Digital-Nervous/mcp-server/server.ts"
    ...
    # Find Digital-Nervous-mcp-server
    server_path="$(node -e "
var fs = require('fs');
var path = require('path');
try {
  var m = JSON.parse(fs.readFileSync('$manifest', 'utf8'));
  var server = (m.servers || []).find(function(s){return s.type==='Digital-Nervous-mcp-server'&&s.enabled;});
  if (server) {
    var sp = path.join('$workspace', '.Digital-Nervous', 'mcp-server', 'server.ts');
...
" 2>/dev/null)" || {
            # Last fallback: local Digital-Nervous server
            local local_server="$workspace/.Digital-Nervous/mcp-server/server.ts"
            if [[ -f "$local_server" ]]; then
                echo "npx tsx $local_server"
                return 0
...
main() {
    ...
    server_cmd="$(resolve_server_cmd "$workspace" "$manifest")" || {
        echo '[Digital-Nervous-launcher] No MCP server found in workspace' >&2
        exit 1
    }
