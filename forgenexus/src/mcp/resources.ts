...
export function registerResources(server: Server, db: ForgeDB, cwd: string): void {
  ...
  server.setRequestHandler(ListResourceTemplatesRequestSchema, async () => ({
    resourceTemplates: [
      {
        uriTemplate: 'forgenexus://repo/{name}/context',
        name: 'Repo Overview',
        mimeType: 'text/yaml',
      },
      {
        uriTemplate: 'forgenexus://repo/{name}/clusters',
        name: 'Repo Modules',
        mimeType: 'text/yaml',
      },
      {
        uriTemplate: 'forgenexus://repo/{name}/processes',
        name: 'Repo Processes',
        mimeType: 'text/yaml',
      },
      {
        uriTemplate: 'forgenexus://repo/{name}/schema',
        name: 'Graph Schema',
        mimeType: 'text/yaml',
      },
      {
        uriTemplate: 'forgenexus://repo/{name}/cluster/{clusterName}',
        name: 'Module Detail',
        mimeType: 'text/yaml',
      },
      {
        uriTemplate: 'forgenexus://repo/{name}/process/{processName}',
        name: 'Process Trace',
        mimeType: 'text/yaml',
      },
      {
        uriTemplate: 'forgenexus://repo/{name}/stats',
        name: 'Detailed Stats',
        mimeType: 'text/yaml',
      },
    ],
...
async function readResource(uri: string, db: ForgeDB, cwd: string): Promise<string> {
  ...
  if (parts[0] === 'repo' && parts[2] === 'context') {
    ...
    const staleness = meta.lastCommit
      ? checkStaleness(cwd, meta.lastCommit)
      : { isStale: false, commitsBehind: 0 }
    const lines: string[] = [`# ${meta.repoName ?? cwd.split('/').pop() ?? 'Repo'}`, '']
  ...
  if (parts[0] === 'repo' && parts[2] === 'processes') {
    ...
    for (const p of procs.slice(0, 20)) {
      ...
      if (p.communities?.length > 0)
        lines.push(`    communities: [${p.communities.slice(0, 3).join(', ')}]`)
    }
    if (procs.length > 20) {
