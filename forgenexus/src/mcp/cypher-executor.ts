  ...
  if (toks[i] === '{') {
    ...
    while (toks[i] && toks[i] !== '}') {
      if (toks[i] === 'name' && (toks[i + 1] === '=' || toks[i + 1] === ':')) {
        // Skip "name:" or "name=" to the VALUE token
        i += 2
        const cur = toks[i]
...
function parseCondition(expr: string): WhereCondition | null {
  ...
  if (m)
    return {
      alias: m[1].split('.')[0],
      prop: m[1].split('.')[1] ?? m[1],
      op: m[2] ? 'IS_NOT_NULL' : 'IS_NULL',
      value: null,
    }

  // CONTAINS / STARTS_WITH / ENDS_WITH
  m = expr.match(
    /^(\w+(?:\.\w+)*)\s+(CONTAINS|STARTS_WITH|ENDS_WITH)\s+(?:"([^"]*)"|'([^']*)'|(\S+))$/i,
  )
  if (m) {
  ...
  while (i < toks.length) {
    ...
    if (t === 'MATCH' || t === 'WITH') {
      ...
      while (
        i < toks.length &&
        toks[i] !== '[' &&
        toks[i]?.toUpperCase() !== 'WHERE' &&
        toks[i]?.toUpperCase() !== 'RETURN' &&
        toks[i]?.toUpperCase() !== 'ORDER' &&
        toks[i]?.toUpperCase() !== 'LIMIT' &&
        toks[i]?.toUpperCase() !== 'WITH'
      ) {
        i++
      }
      if (toks[i] === '[') {
...
function resolveCandidates(
  ...
  return candidates.filter(
    (n) => matchNode(n, centerLabel, centerName) && passesWhere(n, centerWheres),
  )
}
...
function exec(db: ForgeDB, pq: ParsedQuery): CypherResult {
  ...
  const { pattern } = pq
  const returnVars =
    pq.returnVars.length > 0
      ? pq.returnVars
      : [`${pattern.srcAlias}.name`, `${pattern.dstAlias}.name`]
    ...
    for (const center of candidates) {
      ...
      for (const rn of related) {
        ...
        if (pq.where) {
          ...
          const pass =
            pq.where.logic === 'AND' ? condResults.every(Boolean) : condResults.some(Boolean)
          if (!pass) continue
        }
