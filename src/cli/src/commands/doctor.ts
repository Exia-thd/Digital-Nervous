...
async function handleDoctor(options: { verbose: boolean; json: boolean }): Promise<void> {
  ...
  checks.push(checkDigital-Nervous());
...
function checkDigital-Nervous(): HealthCheck {
  // Check if we're in a Digital-Nervous project
  const cwd = process.cwd();
  const Digital-NervousRoot = findDigital-NervousRoot(cwd);

  if (!Digital-NervousRoot) {
    return {
      name: 'Digital-Nervous Project',
      status: 'warning',
      message: 'Not in a Digital-Nervous project',
  ...
  return {
    name: 'Digital-Nervous Project',
    status: 'ok',
    message: `Found at ${Digital-NervousRoot}`,
  };
}

function checkConfig(): HealthCheck {
  const userConfig = resolve(homedir(), '.config', 'Digital-Nervous', 'config.json');
  const legacyConfig = resolve(homedir(), '.Digital-Nervous', 'config.json');
...
function checkMemory(): HealthCheck {
  const memoryPath = resolve(process.cwd(), '.Digital-Nervous', 'memory.jsonl');
...
function findDigital-NervousRoot(cwd: string): string | null {
  let current = cwd;

  while (current !== '/') {
    const configPath = join(current, '.Digital-Nervous');
...
function printHumanReadable(
  ...
  console.log();
  console.log(pc.bold('╔════════════════════════════════════════════════════════════════╗'));
  console.log(pc.bold('║') + '              Digital-Nervous Doctor'.padEnd(62) + pc.bold('║'));
  console.log(pc.bold('╠════════════════════════════════════════════════════════════════╣'));
