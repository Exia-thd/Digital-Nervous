...
Current token tracking only works within Digital-Nervous context. User wants:
1. Track usage across ALL AI assistant platforms
2. Per-project breakdown (not just Digital-Nervous projects)
...
| **Digital-Nervous** | Tokens, cost, skill, mode | ✅ HIGH | `~/.Digital-Nervous/usage/{project}/*.jsonl` |
...
interface UnifiedUsageRecord {
  timestamp: Date;
  platform: 'cursor' | 'claude-code' | 'Digital-Nervous' | 'ollama';
  sessionId: string;
  project: string;
  projectPath: string;
  model: string;
  ...
  inputTokens?: number;    // Digital-Nervous only
  outputTokens?: number;   // Digital-Nervous only
...
- [ ] **Multi-Platform Reader**: Read from Cursor DB, Claude telemetry, Digital-Nervous logs
