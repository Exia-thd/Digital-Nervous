...
├─────────────────────────────────────────────────────────────┤
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────────┐   │
│  │  Cursor DB  │  │ Digital-Nervous │  │  (Future) API   │   │
...
class UnifiedUsageRecord:
    timestamp: datetime
    source: str           # "cursor" | "Digital-Nervous" | "claude-code"
...
### 2. Digital-NervousReader
```python
class Digital-NervousReader:
    def get_usage(self, project: str, days: int = 7) -> List[Dict]:
        """Read usage records from ~/.Digital-Nervous/usage/"""
        base_path = Path.home() / ".Digital-Nervous" / "usage" / project
        # ... read .jsonl files
```
...
class UnifiedAggregator:
    def aggregate(self, cursor_data, Digital-Nervous_data) -> Dict:
        return {
            "by_source": {...},
            "by_model": {...},
            "by_day": {...},
...
def unified_usage():
    ...
    Digital-Nervous = Digital-NervousReader().get_usage(project)
    return jsonify(UnifiedAggregator().aggregate(cursor, Digital-Nervous))
```
