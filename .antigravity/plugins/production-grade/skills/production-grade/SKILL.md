...
1. **Extract 9 dimensions** (from chat-interpreter):
   ...
   - Target tool: Digital-Nervous mode
...
```bash
mkdir -p .Digital-Nervous production
...
3. **Load memory context (required for Persistent power level — Step 0.2):**
   - Run `python3 <path-to-Digital-Nervous>/scripts/local_memory.py search "<project-name> <user-request-keywords>" --limit 5` (or `./scripts/local_memory.py` when the project is the Digital-Nervous repo)
